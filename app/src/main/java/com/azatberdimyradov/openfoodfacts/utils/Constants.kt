package com.azatberdimyradov.openfoodfacts.utils

object Constants {

    const val BASE_URL = "https://world.openfoodfacts.org"
    const val DATABASE_NAME = "open_food_facts_database"

    const val NUTRISCORE_A = "a"
    const val NUTRISCORE_B = "b"
    const val NUTRISCORE_C = "c"
    const val NUTRISCORE_D = "d"
    const val NUTRISCORE_E = "e"

    const val NOVA_1 = 1
    const val NOVA_2 = 2
    const val NOVA_3 = 3
    const val NOVA_4 = 4

    const val ECOSCORE_A = "a"
    const val ECOSCORE_B = "b"
    const val ECOSCORE_C = "c"
    const val ECOSCORE_D = "d"
    const val ECOSCORE_E = "e"

    const val SUCCESS_TIME_DELAY = 1200L

    const val FAT = "Fat"
    const val SALT = "Salt"
    const val SATURATED_FAT = "Saturated fat"
    const val SUGARS = "Sugars"

    const val LOW = "low"
    const val MODERATE = "moderate"
    const val HIGH = "high"

}